...
- (UIButton *)rightBtn
{
    if (!_rightBtn) {
        ...
        _rightBtn.frame = CGRectMake(WK_SCREEN_WIDTH - 100, STATUS_BAR_HEIGHT, 100, 44);
        _rightBtn.tag = 101;
    }
    return _rightBtn;
}
...
- (UILabel *)titleLabel
{
    if (!_titleLabel) {
        ...
        _titleLabel.frame = CGRectMake(CGRectGetMidX(self.frame) - 50, STATUS_BAR_HEIGHT, 100, 44);
...
- (UIButton *)leftBtn
{
    if (!_leftBtn) {
        ...
        _leftBtn.frame = CGRectMake(0, STATUS_BAR_HEIGHT, 60, 44);
        _leftBtn.tag = 100;
...
{
    if (!_line) {
        _line = [[UIView alloc] initWithFrame:CGRectMake(0, STATUS_BAR_HEIGHT + 44, WK_SCREEN_WIDTH, 0.5)];
